...
default_pathologies = ["non-pneumonia", "non-COVID pneumonia", "COVID pneumonia"]
...
class PneumoniaXRayDataset(Dataset):
    ...
    def __init__(self, csvpath, imgpath, transform=None, data_aug=None):
        """
        Args:
            csvpath (string): Path to the csv file with annotations.
        ...
        self.transform = transform
        self.data_aug = data_aug
        self.labels = self.one_hot_encode_labels(self.csv['label'].values)
    
    def one_hot_encode_labels(self, labels):
        # Convert labels to one-hot encoded format
        return np.eye(len(self.pathologies))[labels]
    ...
    def __getitem__(self, index):
        sample = {}
        sample['idx'] = index
        sample['lab'] = self.labels[index]

        # Image path and loading
        row = self.csv.iloc[index]
        img_path = os.path.join(self.imgpath, str(row['label']), row['filename'])
        ...
        # Assigning processed image and metadata to the sample dictionary
        sample['img'] = image
#         sample['metadata'] = {
#             'age': row['age'],
#             'sex': row['sex'],
#             'patientid': row['dataset_ID'],
...
#         }
    ...
    def __str__(self):
        return f"{self.__class__.__name__} - Total samples: {len(self)}, Directory: {self.imgpath}, Labels: {np.unique(self.labels, axis=0)}"
